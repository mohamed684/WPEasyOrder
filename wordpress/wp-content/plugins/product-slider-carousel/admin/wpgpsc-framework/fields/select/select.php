 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'WPGPSC_Field_select' ) ) {
	class WPGPSC_Field_select extends WPGPSC_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ! empty( $args['ajax'] ) ) {
					...
					$args['settings']['data']['nonce'] = wp_create_nonce( 'wpgpsc_chosen_ajax_nonce' );
				...
				$chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' wpgpsc-chosen-sortable' : '';
				$chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' wpgpsc-chosen-ajax' : '';
				$placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '"' : '';
				$field_class      = ( $args['chosen'] ) ? ' class="wpgpsc-chosen' . esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) . '"' : '';
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . $field_name . '" class="wpgpsc-hide-select hidden"' . $multiple_attr . $field_attr . '>';
